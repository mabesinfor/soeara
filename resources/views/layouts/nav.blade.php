    ...
    <div class="lg:hidden">
        ...
        <div class="relative z-30">
            <div class="navbar-burger flex items-center gap-2 cursor-pointer">
                <img src="{{ Auth::user()->avatar ? (filter_var(Auth::user()->avatar, FILTER_VALIDATE_URL) ? Auth::user()->avatar : asset('storage/' . Auth::user()->avatar)) : asset('user.jpg') }}" alt="Profile Picture" class="w-8 h-8 rounded-full">
            </div>
        </div>
        @else
    ...
    @auth
        <div class="hidden lg:block relative z-30">
            <div class="flex items-center gap-2 cursor-pointer" id="profile-dropdown">
                <img src="{{ Auth::user()->avatar ? (filter_var(Auth::user()->avatar, FILTER_VALIDATE_URL) ? Auth::user()->avatar : asset('storage/' . Auth::user()->avatar)) : asset('user.jpg') }}" alt="{{ Auth::user()->name }}" alt="Profile Picture" class="w-8 h-8 rounded-full">
