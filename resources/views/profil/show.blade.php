...
@section('content')
    <section id="profil" class="relative flex py-20 lg:px-40 px-8 justify-center items-center gap-12">
        <img src="{{ $user->avatar ? (filter_var($user->avatar, FILTER_VALIDATE_URL) ? $user->avatar : asset('storage/' . $user->avatar)) : asset('user.jpg') }}" alt="{{ $user->name }}" class="h-80 rounded-full hidden md:block z-20">
        <div class="z-20">
            ...
            <div class="flex text-[#A8A8A8] text-sm gap-4 mb-8">
                ...
                @if ($user->x)
                <div class="">|</div>
                    <a class="flex justify-center items-center gap-2 hover:text-[#C82323]" href="https://twitter.com/{{ $x }}" target="_blank">
                        <svg class="size-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
    ...
    <section id="survei">
        ...
        <div class="relative z-20 flex flex-col items-center gap-10 my-10 lg:mx-40 mx-4">
            <div class="relative z-30 flex flex-col lg:flex-row gap-3 p-3 bg-[#303030]/50 ring-1 ring-[#646464] w-full rounded-xl">
                <div class="relative w-full lg:w-1/2 rounded-lg bg-[#121212]">
                ...
                <div class="w-full lg:w-1/2 rounded-lg bg-[#121212] flex flex-col justify-between">
